...
def _fallback_answer(
    ...
    *,
    ...
    # Техника в генераторския чат — преди слаб QA match
    planner_tech = _planner_technique_answer(low)
    if planner_tech:
        return planner_tech

    qa_hit = _match_qa_card(low)
    if qa_hit:
        return qa_hit
    ...
    if "отскок" in low or "скач" in low or "скочи" in low:
        return (
...
def _match_qa_card(low: str) -> Optional[str]:
    """Строг match — една обща дума като „какво“ не трябва да хваща карта за мач."""
    ...
    for card in qa_cards():
        ...
        keys = [
            ...
            if w
            and w not in _QA_STOP
            and (len(w) > 3 or w in {"мач", "зони", "блок"})
        ]
        if not keys:
            continue
        ...
        if hits < need:
            continue
        # поне половината ключови думи
        if hits * 2 < len(keys) and hits < 3:
            continue
        score = hits * 10 + sum(len(k) for k in keys if k in low)
        ans = str(card.get("a") or "")
        if ans and (best is None or score > best[0]):
            best = (score, ans)
    return best[1] if best else None


def _planner_technique_answer(low: str) -> Optional[str]:
    """Технически отговори за чата в AI генератора (не session_live)."""
    if any(k in low for k in ("кой посрещ", "между двама", "конфликт", "чия е топката", "моят", "моя!")) or (
        "посрещ" in low and any(k in low for k in ("двама", "решава", "кой", "граница"))
    ):
        return (
            "Кой посреща между двама: предварително имате зона на отговорност и зона на конфликт (линията между вас). "
            "Правило на корта: първият, който вика ясно „МОЯ!“, взима топката — другият отваря и асистира. "
            "Ако никой не вика — приоритет на играча в чиято зона е по-голямата част от топката; "
            "при равна граница — по-стабилният/по-близкият до платформата. "
            "Тренирайте с нарочни топки точно на границата и глобявайте мълчанието."
        )
    if "посрещ" in low and any(k in low for k in ("ляво", "наляво", "вляво", "лявата")):
        return (
            "Посрещане „наляво“ като правило: целта е топката след приема да отиде към разпределителя "
            "(обикновено зона 2/3) със стабилна височина — не просто „далеч от мрежата“. "
            "Cue: платформата насочена към целта преди контакта; раменете затворени към желаната линия; "
            "краката под топката, не само ръце. "
            "Грешката е да „избутат“ наляво с рамене назад — тогава топката е ниска/къса. "
            "Тренирайте: сервис → прием към конус до разпределителя → после игра."
        )
    if "посрещ" in low and any(k in low for k in ("правил", "какво знаеш", "как се", "зони")):
        return (
            "За посрещането ключовото е: стабилна платформа, четене на сервис, "
            "ясни зони и глас на границата. "
            "Прогресия: поза → преместване → индивидуален прием → групово с конфликтни зони. "
            "Питай конкретно (наляво, къс сервис, 3 посрещащи) или кажи „генерирай тренировка посрещане“."
        )
    return None
...
Не измисляй медицински диагнози и не предписвай лекарства.
